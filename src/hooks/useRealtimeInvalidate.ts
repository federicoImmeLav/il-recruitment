...
/**
 * Sottoscrizione Supabase Realtime su `bookings`/`mdi`/`notifiche`/`open_day_corsi` filtrata per open day:
...
export function useRealtimeOpenDay(openDayId: string | undefined) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'open_day_corsi', filter: `open_day_id=eq.${openDayId}` },
        () => {
          queryClient.invalidateQueries({ queryKey: ['open_day_corsi'] })
        },
      )
      .subscribe()
